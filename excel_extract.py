...
if 1:
    ...
    for i in list_of_sheets:
        ...
        for j in range(l):
            if pd.notna(df[i].loc[j,"Assignee"]) and pd.notna(df[i].loc[j,"Intel Leads Approval"]) and df[i].loc[j,"Intel Leads Approval"].strip().lower() == "approved":
                #if pd.notna(df[i].loc[j,"Remarks"]):
    ...
    #print("changes jiras are ",changed_jiras)
    df1=pd.DataFrame()
    ...
    file.write("added the adjustment for {} excel file".format(consolidate_approved_jiras))
    ...
    file.write("added the adjustment for {} excel file".format(not_approved_consolidated_jiras))
    
    print("total rows in ",consolidate_approved_jiras," are ",len(approved_jira_df.iloc[:])+1)#1 includes column names
    ...
    try:
        s2 = s1 *[c1_billing,c2_billing,c3_billing,c5_billing]
    except ValueError:
